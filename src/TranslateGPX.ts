...
export default class TranslateGPX {
  ...
  private static calculateDistanceAndElevation(gpx: Gpx, track = 0): Array<Segment> {
    ...
    if (gpx.tracks?.[track]?.trackpoints) {
      ...
      for (let i = 0; i < gpx.tracks[track].trackpoints.length - 1; i++) {
        ...
        const lat2 = gpx.tracks[track].trackpoints[i + 1].lat;
        const lon2 = gpx.tracks[track].trackpoints[i + 1].lon;
        const currentDistance = this.calculateHaversineDistance(lat1, lon1, lat2, lon2) / 1000; // convert to kilometers
  ...
  private static calculateElevations(gpx: Gpx, track = 0): { elevationGain: number; elevationLoss: number } {
    let elevationGain = 0;
    let elevationLoss = 0;
    if (gpx.tracks?.[track]?.trackpoints) {
      for (let i = 0; i < gpx.tracks[track].trackpoints.length - 1; i++) {
  ...
  private static sampleArrayAtIntervals(array: Array<Segment>, sampleSize: number) {
